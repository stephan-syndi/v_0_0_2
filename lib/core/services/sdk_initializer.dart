...
import 'package:v_0_0_2/app.dart';
...
class SdkInitializer {
  ...
  static void showApp(BuildContext context) {
    // We want the transition to the WebView screen to happen only after the
    // page has fully loaded. To do that we temporarily push an offstage
    // preloader that loads the page and notifies when loading finishes.
    // After preload completes we remove the preloader and navigate to the
    // visible WebView screen.

    final String urlToLoad = AppConfig.webGLEndpoint;

    final completer = Completer<void>();

    // Push a non-opaque route that hosts the PreloadWebView (offstage WebView)
    Navigator.of(context).push(PageRouteBuilder(
      opaque: false,
      pageBuilder: (context, animation, secondaryAnimation) => PreloadWebView(
    ...
    // When the preloader signals loaded, pop it and navigate to the real screen
    completer.future.then((_) {
      // remove preload route
      try {
        Navigator.of(context).pop();
      } catch (_) {}

      // Navigate to visible WebView screen
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(builder: (context) => const WebGLWebViewScreen()),
        (route) => false,
      );
    });
  }
